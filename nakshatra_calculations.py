...
def calc_panchangam(
    ...
    if moon_lambda > start_coord:
        ...
        nakshatram_id = int((360 - moon_lambda + start_coord) / nakshatram_extent)
    ...
    for coord_id, coord in enumerate(nakshatram_coords):
        ...
        text_theta = (2 * nakshatram_coords[coord_id] + nakshatram_extent) * (
            np.pi / 360
        )
        if 90 < coord < 270:
    ...
    final_yoga = yoga_names[int(final_yoga_id) - 1]
    ...
    plt.text(
        inv.transform((-200, 500))[0],
        inv.transform((-200, 400))[1],
        "\sam{" + location + "}",
    )

    plt.text(
        inv.transform((-200, 470))[0],
        inv.transform((-200, 370))[1],
        "\sam{" + date_str.split(" ")[0] + "}",
    )
    plt.text(
        inv.transform((-200, 440))[0],
        inv.transform((-200, 340))[1],
        "\sam{" + date_str.split(" ")[1] + "}",
    )

    plt.text(
        inv.transform((-150, 340))[0],
        inv.transform((-150, 340))[1],
        "\sam{पञ्चाङ्ग }",
    ...
    plt.text(
        inv.transform((-200, 310))[0],
        inv.transform((-200, 310))[1],
        tithi_names[int(final_tithi)],
    )
    ...
    plt.text(
        inv.transform((-200, 280))[0],
        inv.transform((-200, 280))[1],
        vaara_names[np.where(weekday == day_of_the_week_at_t)][0],
    )

    plt.text(
        inv.transform((-200, 250))[0],
        inv.transform((-200, 250))[1],
        final_nakshatram + " " + final_padam_text,
    )
